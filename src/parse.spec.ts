...
describe("cookie.parse(str)", function () {
  ...
  it("should parse multiple cookies of same name", function () {
    expect(
      cookie.parse("accessToken=;_jb=2;accessToken=myBeautifulAccessToken.")
    ).toEqual({
      _jb: ["2"],
      accessToken: ["", "myBeautifulAccessToken."],
    });
    expect(cookie.parse("f=;b=;f=")).toEqual({ f: ["", ""], b: [""] });
  });
  ...
  it("should return original value on escape error", function () {
    expect(cookie.parse("foo=%1;bar=bar")).toEqual({
      foo: ["%1"],
      bar: ["bar"],
    });
  });
